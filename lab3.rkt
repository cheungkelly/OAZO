...
#;(define (all-reals? lst)
  (cond
    [(empty? lst) true]  ; An empty list trivially contains all reals
    [(cons? lst) (and (real? (first lst)) (all-reals? (rest lst)))]))  ; Check the first element and recurse
...
(define (ohno val)
  (match val
    [(? number?) 'okay] ; Matches if `val` is a number, returns 'okay
    [_ (error 'ohno "expected a number, got ~a" val)])) ; Matches anything else, raises an error
